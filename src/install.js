...
async function handleRequest(request, event) {
  ...
  if (path === '' || path === '/' || path === '/install') {
    if (request.method === 'POST') {
      ...
      const placementOptions = fdPeek ? String(fdPeek.get('PLACEMENT_OPTIONS') || '').trim() : '';
      const isMarketplace = placementOptions.includes('market');
      const isInstall = authToken && authToken.length > 10 &&
                        (status === 'F' || status === 'L') &&
                        (!placement || (placement === 'DEFAULT' && isMarketplace));
